...
var userId;
console.log(userId); // 특징 1: 혈당전 - undefined (초기화없이 선언 가능)
var userId = "user01"; //특징 2: 재선언 가능 (동일 스코프에서 중복 선언 가능)

userId = "user02"; // 특징 3: 재혈당 - 가능
console.log(userId);
...
// 2. let
let userName;
console.log(userName); // 특징 1. 할당전 _ undefined (초기화없이 선언 가능)
// let userName = "홍길동" // 특징 2. 재선언 - 불가능 (동일 스코프에 중복 전언 붕가능)
...
function letTest() {
  ...
  // console.log(userAge); // 특징4. scope : 블록스코프 (선언된 블록에서만 선언 가능)
}
letTest();

// 3. const - 상수(변경이 불가능한 값)를 선언하기 위해 사용하는 키워드

// const userHobby; // 특징 1. 할당전 - 사용 불가능 (초기화를 해야만 사용 가능)
const userHobby = "요리";
