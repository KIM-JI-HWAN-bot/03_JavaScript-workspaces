...
numbers.sort(); // 숫자 크기대로 오름차순 정렬 x,숫자를 문자열로 변경해서
// 아스키코드 문자 순서로 오름차순 정렬
console.log(numbers);

//숫자 크기대로 오름차순 정렬 => 앞의 숫자가 더 크면 => 순서 변경해야됨 == 양수 변환
...
const lectures = ["HTML", "CSS", "JavaScirpt", "React"];
...
const result = students
  .map(
    (students) =>
      `<li id="students${students.id}">이름: ${students.name}, 나이: ${students.age}</li>`
  )
  .join("");
console.log(result);
